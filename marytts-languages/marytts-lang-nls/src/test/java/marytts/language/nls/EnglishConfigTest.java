...
public class EnglishConfigTest {
	private static final Locale nls = new Locale("nls");
	...
	@Test
	public void canGet() {
		MaryConfig m = MaryConfig.getLanguageConfig(nls);
		assertNotNull(m);
		assertTrue(((LanguageConfig) m).getLocales().contains(nls));
	}

	@Test
	public void hasRussianLocale() throws MaryConfigurationException {
		LanguageConfig e = new EnglishConfig();
		assertTrue(e.getLocales().contains(nls));
	}
}
